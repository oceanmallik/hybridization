...
echo "🔍 Building 5-Language Polyglot Calculator for Linux..."
ARCH=$(uname -m)
...
echo " 1/4 Compiling C Library (Subtraction)..."
gcc -shared -o subtraction.so -fPIC subtraction.c

echo " 2/4 Compiling C# Executable (Multiplication)..."
...
echo " 3/4 Compiling Rust Library (Division)..."
cd division
cargo build --release
cd ..
cp division/target/release/libdivision.so .

echo " 4/4 Compiling C++ Orchestrator (The Brain)..."
